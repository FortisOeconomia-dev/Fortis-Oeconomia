...
const Index = () => {
  const [shouldShowInformativeDialog, setShouldShowInformativeDialog] =
    useState(null);

  useEffect(() => {
    setShouldShowInformativeDialog(
      typeof window !== "undefined" &&
        localStorage.getItem("shouldShowInformativeDialog")
    );
  }, []);
  ...
  return (
    <>
      {shouldShowInformativeDialog == "false" ? (
        <img
          src="./images/fortisoeconomia.png"
          style={{ display: "block", width: "95%", height: "95%" }}
        />
      ) : (
        <InformativeDialog
          onAcceptInformativeDialog={handleAcceptInformativeDialog}
        />
      )}
