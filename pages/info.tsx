...
const LinkLabel = styled.span`
    ...
    background: ${props => props.defaultChecked && 'radial-gradient(35.56% 180% at 50.37% 56.67%, #A8A4F7 0%, #7B61FF 100%)'};
...
const Info = () => {
    ...
    return (
        <Wrapper className='w-full'>
            <LabelWrapper>
                <LinkLabel defaultChecked={selected===0} onClick={() => setSelected(0)}>Assets</LinkLabel>
                <LinkLabel defaultChecked={selected===1} onClick={() => setSelected(1)}>Pools</LinkLabel>
            </LabelWrapper>
            <ContentWrapper>
                <VirticalDivider />
                <div style={{display: 'flex', flexWrap: 'wrap', gap: '72px', justifyContent: 'space-around'}}>
                    ...
                    {selected===0 && 
                        assetData.map((d, idx) => 
                            <Panel data={d} key={`${idx}-panel`} selected={selected}  />
                    )}
                    {selected===1 && pools.map((pool, idx) => 
                        <Panel 
                            data={pool}
                            selected={selected}
                        />
