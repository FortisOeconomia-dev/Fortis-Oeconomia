    ...
    <div style={{ paddingLeft: '27px', maxWidth: '661px', width: '100%' }}>
      {pathname === '/sFOTmodule' && (
        ...
        >
          ...
          >
            {page === 0 ? <span>Clearance Sale<br></br>gFOT to sFOT Swap</span> : 'Stable Module (sFOT)'}
          </button>
          <button
            className={`default-btn  ${!toggle && 'secondary-btn outlined'}`}
            style={{ flex: '1', minWidth: 'unset', borderRadius: '50px' }}
            ...
            return (
              <React.Fragment key={idx}>
                <StatisticItem htmlFor={`${idx}`} slot={`${values.length}`} datatype={pathname}>
                  ...
                  <StatisticValue slot={pathname}> {ConvertToNoExponents(convertToFixedDecimals(v.value))}</StatisticValue>
                </StatisticItem>
